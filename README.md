...
One of the nice things about SQL is that everything is composable. For example, the `pgledger_create_transfer` function only returns the fields on the `pgledger_transfers` table:
...
But if you want other fields, you can still do it in a single query by using common table expressions (CTEs):
